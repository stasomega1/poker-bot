...
Для исправления последней игры пользователь редактирует исходные сообщения и отправляет `/regame` reply на исходную `/game`. Бот берет обновленные тексты из `messageStore`, повторно рассчитывает игру и заменяет последнюю запись, сохраняя ее номер, дату, цену бай-ина, автора и время создания. Более старые игры через `/regame` менять нельзя.
...
- групповые: `/reg`, `/game`, `/regame`, `/setbuyin`, `/bill`, `/debug`, `/app`, `/history`, `/stats`, `/players`, `stats_*`, `/archive`
